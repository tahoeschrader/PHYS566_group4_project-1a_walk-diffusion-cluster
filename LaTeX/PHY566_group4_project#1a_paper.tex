...
\abstract{In our assignment we are using random numbers to study the statistical properties of complex particle systems . Here in particular, we use Morta Carlo methods to simulate 2D random walk and study the statistical parameters of the walkers. In principle, the behavior of the random walkers can be illustrated as a diffusion process satisfying the diffusion equation, which is simulated using the finite difference form model. Furthermore, cluster growth is simulated by diffusion limited aggregation (DLA) model and the 'mass'-radius relation is given in our computation.\\

This is the link to the GitHub account of our project work:\\

https://github.com/tahoeschrader/PHYS566_group4_project-1a_walk-diffusion-cluster}
