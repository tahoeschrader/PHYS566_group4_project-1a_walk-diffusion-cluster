...
\abstract{Numerical methods for simulation of a complex system via random numbers are known as Monte Carlo methods.Here in particular, we use Monte Carlo methods to simulate 2D random walk and study the statistical parameters of the walkers. In principle, the behavior of the random walkers can be illustrated as a diffusion process satisfying the diffusion equation, which is simulated using the finite difference form model. Furthermore, cluster growth is simulated by diffusion limited aggregation (DLA) model and the 'mass'-radius relation is given in our computation.\\
